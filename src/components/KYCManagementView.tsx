  ...
  const fetchKYCDetails = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          employee:employees(name, employee_id, email)
        `)
          ...
          <Card key={kyc.id}>
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-lg">{kyc.employee?.name}</CardTitle>
                  <CardDescription>
                    Employee ID: {kyc.employee?.employee_id} | Email: {kyc.employee?.email}
                  </CardDescription>
                </div>
